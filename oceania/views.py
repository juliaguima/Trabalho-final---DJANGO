...
def editar_comentario(request, id):
    ...
    if request.method == 'POST':
        ...
        if formulario.is_valid():
            ...
            comentario.save()
            return redirect('Oceania:detalhe', link_url=comentario.link_url)
        else:
            formulario = ComentarioForms(initial={'Coment': comentario.coment, 'link_url': comentario.link_url, 'usuario': request.user})
    
    return render(request, 'oceania/paginas/detalhe.html', {'formulario': formulario, 'comentarios': Comentario.objects.all()})
...
def excluir_comentario(request, id):
    ...
    return render(request, 'oceania/paginas/detalhe.html', {'formulario': ComentarioForms(),})
